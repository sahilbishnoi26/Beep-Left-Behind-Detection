...
def callback(frame: np.ndarray, frame_index: int) -> np.ndarray:
    """
    Processes each frame to detect objects and persons, assign ownership, and generate alerts if objects are left behind.
    ...
    detections = detections[detections.confidence > conf_threshold]  # Apply confidence threshold to filter low-confidence detections.

    # Filter detections based on relative area and exclude "tv" class.
    image_area = frame.shape[0] * frame.shape[1]
    detections = detections[(detections.area / image_area) < 0.1]  # Filter out large detections based on area.
    ...
    # Separate person detections from object detections.
...
for video_file in video_files:
    ...
    target_file = f"processed_inside_camera_{video_file}"
